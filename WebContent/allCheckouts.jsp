                    ...
                    <div class="col-lg-12">
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover" id="completeStatusTable">
                                ...
                                <tbody>
                                <%List completeStatus =  (List) request.getAttribute("allCheckouts");%>
                                    <%  for (Iterator iterator = 
                                    		completeStatus.iterator(); iterator.hasNext();){
             User user = (User) iterator.next(); 
             %>
             <tr class="completeStatusTableRow">
             <td class="completeStatusTableRowId"><%= user.getId()  %></td>
             <td class="completeStatusTableRowName"><%= user.getName() %></td>
             <td class="completeStatusTableRowPhoneNumber"><%= user.getPhoneNumber() %></td>
             <td class="completeStatusTableRowCity"><%= user.getCity() %></td>
             <td class="completeStatusTableRowComingDate"><%= user.getComingDate() %></td>
             <td class="completeStatusTableRowReturnDate"><%= user.getReturnDate() %></td>
             <td class="completeStatusTableRowNumberOfPeople"><%= user.getNumberOfPeople() %></td>
             <td class="completeStatusTableRowRoomAlloted"><%= user.getRoomAlloted() %></td>
             </tr>
             <%
          }%>
                                </tbody>
